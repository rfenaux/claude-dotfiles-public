#!/usr/bin/env python3
"""
CSB Bash Guard - PreToolUse Hook for Bash Tool

Checks taint status before allowing Bash operations.
- CRITICAL taint: HARD BLOCK (exit 2)
- HIGH taint: Request permission (permissionDecision: ask)
- Cleared taint: Allow operation

Note: This runs AFTER the outgoing-data-guard.py which checks for
sensitive data in outgoing requests. This guard checks for taint
from previously read dangerous content.
"""
...
def main():
    ...
    target = command[:60] + "..." if len(command) > 60 else command
    ...
    if taint["level"] == "HIGH":
        message = format_ask_message(taint, "Bash", target)
        print(message, file=sys.stderr)
